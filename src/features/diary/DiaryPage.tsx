...
import { Check, CheckSquare, ChevronLeft, ChevronRight, Sparkles } from 'lucide-react'
import { addDays, addMonths, endOfMonth, endOfWeek, format, isSameMonth, isValid, parseISO, startOfMonth, startOfWeek } from 'date-fns'
...
function DiaryEditor() {
  ...
  const [visibleMonth, setVisibleMonth] = useState(() => startOfMonth(parseISO(entryDate)))
...
function DiaryCalendar({ month, selected, today, writtenDates, onMonthChange, onSelect }: { month: Date; selected: string; today: string; writtenDates: Set<string>; onMonthChange: (amount: number) => void; onSelect: (date: string) => void }) {
  ...
  return <section className="diary-calendar" aria-label="다이어리 달력"><header><button type="button" onClick={() => onMonthChange(-1)} aria-label="이전 달"><ChevronLeft size={15}/></button><b>{format(month, 'yyyy년 M월')}</b><button type="button" onClick={() => onMonthChange(1)} aria-label="다음 달"><ChevronRight size={15}/></button></header><div className="diary-calendar-week">{['일','월','화','수','목','금','토'].map((day) => <span key={day}>{day}</span>)}</div><div className="diary-calendar-days">{days.map((day) => { const key = format(day, 'yyyy-MM-dd'); const written = writtenDates.has(key); return <button type="button" key={key} disabled={key > today} className={`${key === selected ? 'selected' : ''} ${!isSameMonth(day, month) ? 'outside' : ''} ${written ? 'written' : ''}`} onClick={() => onSelect(key)} aria-label={`${format(day, 'M월 d일')}${written ? ', 다이어리 작성됨' : ''}`}><span>{format(day, 'd')}</span>{written && <i aria-hidden="true"/>}</button> })}</div><p><i/> 다이어리를 작성한 날</p></section>
}
