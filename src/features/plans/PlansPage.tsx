...
export function PlansPage() {
  ...
  return <section className="plans-page">
    ...
    {catalog.length > 0 && <section className="catalog-section"><header><span className="eyebrow">PALACE COLLECTION</span><h2>테마·위젯 컬렉션</h2></header><div>{catalog.map((item) => <article className="panel glass-panel" key={item.productKey}><span>{item.category === 'theme' ? '궁전 테마' : '위젯'}</span><h3>{item.title}</h3><p>{item.description}</p><b>{item.priceKrw.toLocaleString()}원</b></article>)}</div></section>}
  </section>
}
