...
function AlternativeCalendarView({ mode, events, selected, onSelect }: { mode: Exclude<CalendarView, '월간'>; events: CalendarEvent[]; selected: string; onSelect: (date: string) => void }) {
  ...
  const filtered = mode === '일간' ? events.filter((event) => eventOccursOn(event, selected))
    : mode === '주간' ? events.filter((event) => parseISO(event.date) <= rangeEnd && parseISO(event.endDate ?? event.date) >= rangeStart)
      : events
...
function eventOccursOn(event: CalendarEvent, date: string) { return event.date <= date && (event.endDate ?? event.date) >= date }
