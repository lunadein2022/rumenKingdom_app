...
export function CalendarPage() {
  ...
  return <div className="calendar-shell glass-panel">
    ...
    <aside className="day-panel">
      ...
      <div className="agenda-list">{selectedEvents.length ? selectedEvents.map((event) => <div className="agenda-item" key={event.id}><i className={kindClass[event.kind]} /><div><span>{event.start || '하루 종일'}{event.end ? ` — ${event.end}` : ''}</span><b>{event.title}</b><small>{calendarKinds.find((kind) => kind.id === event.kind)?.label}</small></div><button onClick={() => openEdit(event)} aria-label={`${event.title} 편집`}><Pencil size={14} /></button><button onClick={() => deleteEvent(event.id)} aria-label={`${event.title} 삭제`}><Trash2 size={14} /></button></div>) : <EmptyMini onCreate={openCreate} />}</div>
...
function MonthGrid({ month, events, selected, onSelect, onMove, onOpen }: { month: Date; events: CalendarEvent[]; selected: string; onSelect: (date: string) => void; onMove: (id: string, date: string) => void; onOpen: (event: CalendarEvent) => void }) {
  ...
  return <div className="month-grid"><div className="weekday-row">{['일','월','화','수','목','금','토'].map((day) => <span key={day}>{day}</span>)}</div><div className="date-grid" style={{ gridTemplateRows: `repeat(${days.length / 7}, 1fr)` }}>{days.map((day) => {
    ...
    return <div key={date} role="button" tabIndex={0} aria-label={`${format(day, 'M월 d일')}, 일정 ${dayEvents.length}건`} className={`date-cell ${date === selected ? 'selected' : ''} ${!isSameMonth(day, month) ? 'outside' : ''}`} onClick={select} onKeyDown={(event) => (event.key === 'Enter' || event.key === ' ') && select()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => onMove(event.dataTransfer.getData('text/plain'), date)}><span className="date-number">{format(day, 'd')}</span><div className="event-stack">{dayEvents.slice(0, 3).map((event) => <button type="button" draggable onDragStart={(drag) => drag.dataTransfer.setData('text/plain', event.id)} onClick={(click) => { click.stopPropagation(); onOpen(event) }} key={event.id} className={`event-chip ${kindClass[event.kind]}`} aria-label={`${event.title} 상세`}><time>{event.start}</time>{event.title}</button>)}{dayEvents.length > 3 && <small>+{dayEvents.length - 3}개 더</small>}</div></div>
  })}</div></div>
}
...
function AlternativeCalendarView({ mode, events, selected, onSelect }: { mode: Exclude<CalendarView, '월간'>; events: CalendarEvent[]; selected: string; onSelect: (date: string) => void }) {
  ...
  const filtered = mode === '일간' ? events.filter((event) => event.date === selected)
    : mode === '주간' ? events.filter((event) => { const date = parseISO(event.date); return date >= rangeStart && date <= rangeEnd })
      : events
  return <div className="alternative-view"><div className="alternative-title"><CalendarDays size={21} /><span>{mode} 보기</span></div>{filtered.length ? filtered.map((event) => <button key={event.id} onClick={() => onSelect(event.date)}><i className={kindClass[event.kind]} /><time>{format(parseISO(event.date), 'M월 d일 (EEE)', { locale: ko })} · {event.start || '하루 종일'}</time><b>{event.title}</b><ChevronRight size={15} /></button>) : <div className="calendar-empty"><CalendarDays size={28}/><b>표시할 일정이 없습니다.</b><span>다른 날짜나 필터를 선택해 보세요.</span></div>}</div>
}
...
function EventModal({ date, initial, onClose, onSave }: { date: string; initial?: CalendarEvent; onClose: () => void; onSave: (event: Omit<CalendarEvent, 'id'>) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="modal glass-panel" role="dialog" aria-modal="true" aria-labelledby="event-modal-title" onSubmit={(event) => { event.preventDefault(); if (title.trim()) onSave({ title: title.trim(), date: eventDate, start, end: end || undefined, kind, important, recurrenceRule: recurrenceRule || undefined }) }} onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">{initial ? 'EDIT SCHEDULE' : 'NEW SCHEDULE'}</span><h2 id="event-modal-title">{initial ? '일정 상세 편집' : '새 일정 만들기'}</h2></div><button type="button" aria-label="닫기" onClick={onClose}><X size={18} /></button></div><label>일정 이름<input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder="어떤 일정인가요?" required /></label><div className="form-row"><label>날짜<input type="date" value={eventDate} onChange={(event) => setEventDate(event.target.value)} /></label><label>시작 시간<input type="time" value={start} onChange={(event) => setStart(event.target.value)} /></label></div><div className="form-row"><label>종료 시간<input type="time" value={end} min={start} onChange={(event) => setEnd(event.target.value)} /></label><label>반복<select value={recurrenceRule} onChange={(event) => setRecurrenceRule(event.target.value)}><option value="">반복 안 함</option><option value="FREQ=DAILY">매일</option><option value="FREQ=WEEKLY">매주</option><option value="FREQ=MONTHLY">매월</option></select></label></div><label>캘린더<select value={kind} onChange={(event) => setKind(event.target.value as CalendarKind)}>{calendarKinds.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><label className="important-check"><input type="checkbox" checked={important} onChange={(event) => setImportant(event.target.checked)}/> 중요 일정으로 표시</label><div className="modal-actions"><button type="button" className="ghost" onClick={onClose}>취소</button><button type="submit" className="primary">{initial ? <Pencil size={15} /> : <Plus size={15} />} {initial ? '변경사항 저장' : '일정 추가'}</button></div></form></div>
}
