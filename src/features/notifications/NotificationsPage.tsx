...
export function NotificationsPage() {
  ...
  return <div className="notifications-page">
    ...
    <section className="panel glass-panel notifications-panel">
      ...
      {!notificationsEnabled ? <p className="notifications-empty">왕좌의 방에서 앱 내부 알림이 꺼져 있습니다.</p> : visible.length ? <div className="notifications-list">{visible.map((item) => <button key={item.id} className={item.read ? 'read' : 'unread'} onClick={() => { markRead(item); navigate(item.path) }}>
        <i aria-hidden="true"/>
