...
function EmptyState({ label }: { label: string }) {
  return (
    <article className="library-empty-state">
      <strong>{label}</strong>
      <span>기록이 쌓이면 이곳에서 차분히 다시 찾아볼 수 있습니다.</span>
    </article>
  );
}
  ...
  return (
    <section className="library-page simple">
      <header className="library-hero">
        ...
        <h1>왕국도서관</h1>
        <p>완료 Quest, 지난 일정, Diary, 인연록, 세린의 장기 기억이 모이는 독립 보관소입니다.</p>
      </header>
      ...
      {activeTab === "events" && (
        <section className="library-section">
          <div className="library-section-head"><h2>일정 기록</h2><span>{scopedEvents.length}개</span></div>
      ...
      {activeTab === "diary" && (
        <section className="library-section">
          ...
          <EmptyState label="아직 Diary 기록이 없습니다." />
        </section>
      )}
