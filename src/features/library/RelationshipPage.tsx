...
export function RelationshipPage() {
  ...
  const save = async () => {
    ...
    if (!input.name) { setFormError('이름을 입력해 주세요.'); return }
    setFormError('')
    setSaving(true)
    try {
      if (relationship) {
        if (await updateRelationship(relationship.id, input)) setEditing(false)
        else setFormError('인연록을 저장하지 못했어요. 잠시 후 다시 시도해 주세요.')
      } else {
        const id = await addRelationship(input)
        if (id) {
          clearPersistentState(dk('name'), dk('organization'), dk('position'), dk('phone'), dk('email'), dk('address'), dk('social'), dk('relationshipType'), dk('firstMetAt'), dk('lastContactedAt'), dk('memo'), dk('tags'), dk('groupIds'), dk('favorite'))
          navigate(`/library/relationships/${id}`, { replace: true })
        } else setFormError('인연록을 저장하지 못했어요. 연결 상태나 왕국 DB 설정을 확인해 주세요.')
      }
