...
export function RelationshipPage() {
  ...
  const save = async () => {
    ...
    else { const id = await addRelationship(input); if (id) { clearPersistentState(dk('name'), dk('organization'), dk('position'), dk('phone'), dk('email'), dk('address'), dk('social'), dk('relationshipType'), dk('firstMetAt'), dk('lastContactedAt'), dk('memo'), dk('tags'), dk('favorite')); navigate(`/library/relationships/${id}`, { replace: true }) } }
  }
