...
export function MemoPage() {
  ...
  const [content, setContent] = useState(memo?.content ?? draft?.content ?? '')
  const [tags, setTags] = useState(memo?.tags.join(', ') ?? draft?.tags?.join(', ') ?? '')
  const [projectId, setProjectId] = useState(memo?.projectId ?? '')
  const [important, setImportant] = useState(memo?.important ?? false)
  const [favorite, setFavorite] = useState(memo?.favorite ?? false)
  const [status, setStatus] = useState<MemoStatus>(memo?.status ?? 'normal')
