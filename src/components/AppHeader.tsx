...
export function AppHeader({ page, onMenu, onSignOut }: { page: PageId; onMenu: () => void; onSignOut: () => Promise<void> }) {
  ...
  return <>
    <header className="topbar glass-panel" data-page={page}>
      ...
      <div className="header-actions">
        ...
        <button className="header-shortcut logout-shortcut" aria-label="로그아웃" onClick={() => void onSignOut()}><LogOut size={17}/><span>로그아웃</span></button>
        <button className="menu-button" aria-label="메뉴" onClick={onMenu}><Menu size={20}/></button>
      </div>
      {notificationOpen && <aside className="notification-popover glass-panel"><div><b>왕실 알림</b><button onClick={() => setNotificationOpen(false)} aria-label="알림 닫기"><X size={14}/></button></div><p>오늘 일정 {events.filter((event) => event.date === new Date().toISOString().slice(0, 10)).length}건 · 미완료 퀘스트 {quests.filter((quest) => !quest.done).length}건</p><p>확인할 비망록 {memos.filter((memo) => memo.status === 'review').length}건</p></aside>}
    </header>
