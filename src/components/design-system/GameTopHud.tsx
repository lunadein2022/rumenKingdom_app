  ...
  return (
    <header className="game-topbar">
      ...
      <div className="game-topbar-level">
        <span className="game-topbar-level-badge">♛ Lv.{progress.level} {princess.displayName}</span>
        <div className="game-topbar-level-copy">
          <span>
            EXP {progress.currentExp.toLocaleString()} / {progress.requiredExp.toLocaleString()}
          </span>
          <div className="game-topbar-exp-track">
            <div className="game-topbar-exp-fill" style={{ width: `${progress.expRate}%` }} />
          </div>
        </div>
        <em className="game-topbar-title-label">{princess.activeTitle}</em>
        {onSignOut && (
          <button type="button" className="game-topbar-signout" onClick={onSignOut} aria-label="로그아웃" title="로그아웃">
            ⏻
          </button>
        )}
      </div>
