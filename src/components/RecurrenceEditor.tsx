...
export function RecurrenceEditor({ value, anchorDate, onChange }: { value?: string; anchorDate: string; onChange: (value: string) => void }) {
  const safeAnchorDate = /^\d{4}-\d{2}-\d{2}$/.test(anchorDate) ? anchorDate : new Date().toISOString().slice(0, 10)
  const parsed = useMemo(() => parseRecurrenceRule(value), [value])
  const frequency = parsed.frequency ?? ''
  const anchorWeekday = weekdayCode(safeAnchorDate)
  const anchor = new Date(`${safeAnchorDate}T12:00:00`)
  const byDay = parsed.byDay.length ? parsed.byDay : [anchorWeekday]
  const bySetPos = parsed.bySetPos ?? monthlyPosition(safeAnchorDate)
  ...
  return <fieldset className="recurrence-editor">
    ...
    {frequency && <label>반복 종료일 <small>선택하지 않으면 계속 반복됩니다.</small><input type="date" min={safeAnchorDate} value={parsed.until ?? ''} onChange={(event) => update(frequency, { until: event.target.value })}/></label>}
  </fieldset>
}
