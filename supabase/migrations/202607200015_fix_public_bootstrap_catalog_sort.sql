-- 013/014 selected app_catalog_items into the "item" subquery without sort_order
-- but still ordered by item.sort_order, so every get_public_app_bootstrap call
-- raised 42703 (column item.sort_order does not exist) and clients silently
-- fell back to bundled defaults for config, announcements, catalog and releases.
    ...
    'catalog', coalesce((
      ...
      from (
        select product_key, category, title, description, price_krw,
               entitlement_key, image_url, platforms, metadata, sort_order
